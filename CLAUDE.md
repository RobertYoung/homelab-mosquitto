...
Single-host Ansible project deploying Mosquitto MQTT broker via Docker Compose on `mosquitto.local.iamrobertyoung.co.uk`. Secrets are stored in AWS SSM Parameter Store.
...
```bash
# Test connectivity
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible mosquitto -m ping

# Run full playbook
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible-playbook playbooks/site.yml

# Run specific role only
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible-playbook playbooks/site.yml --tags mosquitto
...
8. `mosquitto` - Mosquitto MQTT broker deployment (Docker Compose with systemd service)

### Mosquitto Role Structure
The `roles/mosquitto` role deploys Mosquitto via Docker Compose:
- `tasks/main.yml` - Main task orchestration (setup, mosquitto, backup)
- `defaults/main.yml` - Default variables
- `templates/` - Jinja2 templates for configuration
- `handlers/main.yml` - Service restart handlers

Key directories on target host:
- `/etc/mosquitto` - Configuration files and docker-compose.yml
- `/var/lib/mosquitto` - Application data
...
### Available Tags
Run specific parts with `--tags`: `configure-system`, `shell`, `docker`, `telegraf`, `step-ca-client`, `syslog`, `wazuh-agent`, `mosquitto`
